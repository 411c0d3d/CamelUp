...
public class EngineParticipant {
    ...
    public EngineParticipant(int engineId){
        this.engineId = engineId;
    }

    /**
     * engineTurnDecision decides which move the Engine Participant is going to do, by splitting the game into two phases.
     * The later phase of the game is determined by the turns that have been made divided by the max turns set in gameConfig,
     * which starts at 70% of maxTurns. The winnerBet/looserBet will be chosen by helping methods and handed over to another
     * method that places the determined FinalBet, if not been placed yet (checks current gameState).
     * otherwise a different play will be decided by a different method,
     * based on current gameState like rolling the dice in 90% of the cases or placing a player card in 10%
     * @param gameState: GameStateDTO
     */
    public void engineTurnDecision(GameStateDTO gameState){
    ...
    //habe ich die schon benutzt, warum steht das nicht im Gamsestate aber alle Kamele mit ID AAAAAAAAAAHHHHHH
     ...
     * @return boolean
     */
    public boolean canPlacePlayerCard(GameStateDTO gameState){
        List<BoardSpaceDTO> fields = gameState.getBoardSpaces();
        for (BoardSpaceDTO field : fields){
    ...
    /**
     * Decides if the Engine is rolling or placing a player card at 70% of the Etappe
     * @param gameState GameStateDTO
     * @return
     */
    private void calculateEtappenPlay(GameStateDTO gameState) {
        int camelAmount = gameState.getGameConfig().getCamelCount();
    ...
    /**
     * Finds out the closest camel or Rudel towards the finish line, by calculating the distance difference of the
     * current leading camel or Rudel and possible other camels or Rudels, being possibly able to catch up with
     * their maximum dice power. If it's less than the difference the leading camel or Rudel need to win, consider them
     * as the possible winner of the Game. If it's distance is bigger, choose leading camel or Rudel to bet on.
     * @param gameState GameStateDTO
     * @return possibleWinners a list of all possible winners
     */
    public List<BoardSpaceDTO> calculatePossibleWinners(GameStateDTO gameState) {
        List<BoardSpaceDTO> possibleWinners = new ArrayList<>();
        int distance = 0;
        int powerOfRudel = 0;
        ...
        for (int i = firstPlaceField-1; i >= 0; i--) {
            int place = i +1;
            BoardSpaceDTO boardSpace = gameState.getBoardSpaces().get(place);
            ...
            if (distance <= powerOfRudel) {
                ...
                for (int camel : boardSpace.getCamelIds()) {
                    if (movableCamels.contains(camel)) {
                        powerOfRudel += gameState.getGameConfig().getDiceFacesCount();
                    }
                }
    ...
    /**
     * calculates the winner of an "etappe" and for the final bet -> Miro contains exact documentation on the logic
     * @param gameState GameStateDTO
     * @return winnerID: winner out of list of possible winners
     */
    public int calculateCamelToBetOn(GameStateDTO gameState){
        int winnerId = 0;
        List<BoardSpaceDTO> possibleWinners = calculatePossibleWinners(gameState);
        if (possibleWinners.size() == 1 && possibleWinners.get(0).getCamelIds().size() == 1){
            ...
            List<Integer> movedCamels = new ArrayList<>(gameState.getRolledDice().stream().map(RolledDiceDTO::getCamelId).toList());
            ...
            for (BoardSpaceDTO boardSpace : possibleWinners) {
                int sizeOfStack = boardSpace.getCamelIds().size();
                List<Integer> CamelIds = boardSpace.getCamelIds();
                CamelIds.removeAll(movedCamels);
                int powerOfRudel = CamelIds.size()*gameState.getGameConfig().getDiceFacesCount();
                if (powerOfRudel > biggestStack){
                    winnerIdOfBiggestStack = boardSpace.getCamelIds().get(sizeOfStack - 1);
     ...
     * places the bet or decides to go for another play if not and references the methods
     * @param gameState
     */
    public void calculateBettingOrRolling(GameStateDTO gameState){
        int bettingCamelID = calculateCamelToBetOn(gameState);
        for (BettingCardDTO bettingCard: gameState.getBettingCards()){
            if (bettingCard.getCamelId() == bettingCamelID && bettingCard.getWorth() > 0){
    ...
    /**
     * finds the boardSpace with a non backwards camel on, which is closest to the goal
     * @param gameState BoardSpaceDTO
     * @return boardSpaceId in question
     */
    public int findCurrentFirstPlaceField(GameStateDTO gameState){
        boolean isNotBackwards = false; //make sure no backwards camel is used
        int boardSpaceId = 0;
        for (int i = gameState.getBoardSpaces().size() - 1; i >= 0; i--) {
            BoardSpaceDTO boardSpace = gameState.getBoardSpaces().get(i);
            if (boardSpace.getCamelIds().isEmpty()){
                continue;
            }
     ...
     */
    public List<Integer> calculateAllMovedCamels(GameStateDTO gameState) {
        List<Integer> allMovedCamelIds = new ArrayList<>();
        if (gameState.getRolledDice() != null) {
            for (RolledDiceDTO roll : gameState.getRolledDice()) {
     ...
     */
    public List<Integer> calculateAllCamelIds(GameStateDTO gameState) {
        List<Integer> allCamelIds = new ArrayList<>();
        int camelCount = gameState.getGameConfig().getCamelCount();
        for (int id = 0; id < camelCount; id++) {
            allCamelIds.add(id);
        }
        return allCamelIds;
    }
     ...
     */
    public List<Integer> calculateCamelsAbleToMove(GameStateDTO gameState){
     ...
     */
    public int calculatePossibleLooser(GameStateDTO gameState){
        int looserId = -1;
        int distanceTracker = 0;
        for (BoardSpaceDTO boardSpace : gameState.getBoardSpaces()) {
            if (distanceTracker > gameState.getGameConfig().getDiceFacesCount()) {
                break;
            }
     ...
     */
    public boolean decideFinalBet(GameStateDTO gameState){
        int winnerBet = calculateCamelToBetOn(gameState);
        int looserBet = calculatePossibleLooser(gameState);
        ...
        for (int i = looserField+1; i < gameState.getBoardSpaces().size(); i++){
            BoardSpaceDTO field = gameState.getBoardSpaces().get(i);
     ...
     */
    public List<Integer> calculateYourPlacedBets(GameStateDTO gameState){
        List<Integer> placedBets = new ArrayList<>();
        List<FinalBetDTO> winnerBets = gameState.getFinalBets().getFirstCamel();
        List<FinalBetDTO> looserBets = gameState.getFinalBets().getLastCamel();
        winnerBets.addAll(looserBets);
        for (FinalBetDTO finalBet : winnerBets) {
            if (finalBet.getCamelId() == engineId) {
                placedBets.add(finalBet.getCamelId());
            }
        }
        return placedBets;
    }
     ...
     * @param gameState GameStateDTO
     */
    public void rollTheDice(GameStateDTO gameState){
        // TODO ROLLING DICE an server schicken
    }
     ...
     * @return
     */
    public FinalBetDTO placeFinalBet(Boolean isFirst, int id){
        FinalBetDTO finalBet = new FinalBetDTO();
        finalBet.setPlayerId(engineId);
        finalBet.setCamelId(id);
     ...
     * @return
     */
    public StageBetDTO placeStageBet(int camelId){
        StageBetDTO stageBet = new StageBetDTO(camelId);
        //TODO als DTO verschicken
