...
public class Api {
    ...
    private <T> void completePendingRequest(final Packet packet, final Class<T> responseClass) {
        ...
        if (content instanceof final SuccessFeedback feedback) {
            System.out.println("<-- Received feedback response for request \""+feedback.getRequest().getType());
            request = pendingFeedbackRequests.remove(feedback.getRequest().getType().value());
            if (!feedback.getSuccess()) {
                throw new IllegalStateException("Request " + feedback.getRequest() + " was not successfull. Error: " + feedback.getError());
            }
        } else {
            System.out.println("<-- Received response of type "+content.getClass().getSimpleName()+" for enum "+packet.getType() +"with content: "+ content);
            request = pendingRequests.remove(responseClass);
        }

        if (request == null) {
            System.out.println("Found no pending request future for type "+content.getClass().getSimpleName() +" with content: "+ content);
            return;
        }
    ...
    public CompletableFuture<ClientAck> connect(final String host, final int port) throws IOException {
        System.out.println("--> Connecting to \""+host == null ? "localhost" : host+":"+port +"\" ...");
    ...
    public CompletableFuture<SuccessFeedback> registerPlayer(final String name) {
        System.out.println("--> Registering player as \""+name+"\" ...");
        var future = new CompletableFuture<SuccessFeedback>();
        addPendingFeedbackRequest(Packet.Type.PLAYER_REGISTRATION, future);

        client.sendMessage(Packet.builder()
            .withType(Packet.Type.PLAYER_REGISTRATION)
    ...
    public CompletableFuture<SuccessFeedback> placeFinalBet(final int camelId, final boolean isFirst) {
        System.out.println("--> Placing final bet on camel "+camelId+", is first "+isFirst +" ...");
        var future = new CompletableFuture<SuccessFeedback>();
        addPendingFeedbackRequest(Packet.Type.FINAL_BET, future);

        client.sendMessage(Packet.builder()
            .withType(Packet.Type.FINAL_BET)
    ...
    public CompletableFuture<SuccessFeedback> rollDice() {
        ...
        var future = new CompletableFuture<SuccessFeedback>();
        addPendingFeedbackRequest(Packet.Type.ROLL_DICE, future);

        client.sendMessage(Packet.builder()
            .withType(Packet.Type.ROLL_DICE)
    ...
    public CompletableFuture<SuccessFeedback> betOnStage(final int camelId) {
        System.out.println("--> Betting on stage on camel "+camelId+" ...");
        var future = new CompletableFuture<SuccessFeedback>();
        addPendingFeedbackRequest(Packet.Type.STAGE_BET, future);

        client.sendMessage(Packet.builder()
            .withType(Packet.Type.STAGE_BET)
    ...
    public CompletableFuture<SuccessFeedback> joinLobby(int lobbyId, boolean joinAsPlayer) {
        //System.out.println("--> Joining lobby {} as player: {} ...".formatted(lobbyId, joinAsPlayer));
        var future = new CompletableFuture<SuccessFeedback>();
        addPendingFeedbackRequest(Packet.Type.JOIN_LOBBY, future);

        client.sendMessage(Packet.builder()
            .withType(Packet.Type.JOIN_LOBBY)
    ...
    public CompletableFuture<SuccessFeedback> moveVisualized() {
        System.out.println("--> Visualized move ...");
        var future = new CompletableFuture<SuccessFeedback>();
        addPendingFeedbackRequest(Packet.Type.MOVE_VISUALIZED, future);

        client.sendMessage(Packet.builder()
            .withType(Packet.Type.MOVE_VISUALIZED)
    ...
    public CompletableFuture<SuccessFeedback> placePlayerCard(final int spaceId, final int movingDirection) {
        System.out.println("--> Placing player card on "+spaceId+" ...");
        var future = new CompletableFuture<SuccessFeedback>();
        addPendingFeedbackRequest(Packet.Type.PLACE_PLAYER_CARD, future);

        client.sendMessage(Packet.builder()
            .withType(Packet.Type.PLACE_PLAYER_CARD)
    ...
    public CompletableFuture<LobbyList> listLobbies() {
        ...
        client.sendMessage(Packet.builder()
            .withType(Packet.Type.REQUEST_LOBBY_LIST)
